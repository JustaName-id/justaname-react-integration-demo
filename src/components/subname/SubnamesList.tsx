            ...
            <div key={index} className="flex items-center justify-between p-4 bg-muted/50 rounded-lg border">
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground mt-1">
                  ...
                  {subname.records?.texts && Object.keys(subname.records.texts).length > 0 ? (
                    `${Object.keys(subname.records.texts).length} text record(s)`
                  ) : (
                    'No text records'
                  )}
                  {/* Display address record count if any exist */}
                  {subname.records?.coins && Object.keys(subname.records.coins).length > 0 && (
                    ` • ${Object.keys(subname.records.coins).length} address record(s)`
                  )}
                </div>
              </div>
