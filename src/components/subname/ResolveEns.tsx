    ...
    <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent className="space-y-6">
              ...
              {isRecordsFetching ? (
                ...
                <div className="space-y-6">
                  ...
                  {records.records.texts &&
                    Object.keys(records.records.texts).length > 0 && (
                      <div className="space-y-3">
                        <h4 className="text-lg font-semibold">Text Records</h4>
                        <div className="grid gap-3">
                          {Object.entries(records.records.texts).map(
                            (
                  ...
                  {records.records.coins &&
                    Object.keys(records.records.coins).length > 0 && (
                      <div className="space-y-3">
                        <h4 className="text-lg font-semibold">
                          Address Records
                        </h4>
                        <div className="grid gap-3">
                          {Object.entries(records.records.coins).map(
                            (
                              [key, coinData]: [string, any] // eslint-disable-line @typescript-eslint/no-explicit-any
                            ) => {
                              // Extract coinType - it might be nested in the object
                              const coinType =
                                coinData?.coin_type ||
                                coinData?.coinType ||
                                coinData?.id ||
                                key;
                              const address =
                                typeof coinData === "string"
                                  ? coinData
                                  : coinData?.value || coinData?.address || "";
                    ...
                    !records.records.contentHash && (
                      <div className="text-center py-8 text-muted-foreground">
                        ...
                        <p className="text-sm mt-1">
                          This ENS name exists but has no associated records.
                        </p>
                      </div>
                    )}
                </div>
        ...
        <div className="text-xs text-muted-foreground text-center">
          <p>Enter any ENS name to view its public records.</p>
        </div>
      </CardContent>
    </Card>
  );
